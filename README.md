...
On April 30, 2026, this directory was used to compare the local GGUF variants by running the same prompt through each runnable model with the locally installed llama.cpp tools. On May 4, 2026, the three remaining 135-byte Git LFS pointer files were fetched from the Hugging Face remote, materialized as local GGUF files, and added to the same result set.
